...
* `greetings` containing code of the program we will work with. There is hierarchy
  of greeting instances taken from interface `Task3GreetingsProcessor`.
...
* checker needs to know about classes used in the Byteman rules. Classpath `-cp`
  will need to be adjusted on that.
...
* Byteman can make loaded jars by boot class loader (http://downloads.jboss.org/byteman/4.0.0/byteman-programmers-guide.html#available-javaagent-options)
  with argument `bood:<path-to-jar-file>`
